...
def mfq(process_list):
    ...
    def shortest_remaining():
        ...
        for i in range(1, len(ready_queue_2)):
            if ready_queue_2[i].remaining_cpu_burst < shortest.remainig_cpu_burst:
    ...
    def fetch():
        ...
        if len(ready_queue_0) > 0:
            if current_queue == 2: # q2 process가 돌고있는 경우
            ...
            elif current_process.remaining_cpu_burst > shortest.remainig_cpu_burst:
        ...
        for process in process_list:
            ...
            if process is current_process:
                if process.remaining_cpu_burst == 0:  # cpu burst time 0이면
                    ...
                    if process.current_queue == 0 or process.current_queue == 1:  # 이 cpu가 q0 또는 q1에서 왔는지 확인
    ...
    #### 함수 끝날 때 프로세스마다 갖고있는 시간 전부 계산해야됨
